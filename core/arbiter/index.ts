...
export function evaluateGates(request: ExecutionRequest): {
  ...
  if (request.schedulerConflict) {
    gateTrace.push({
      gate: "scheduler_arbitration",
      decision: "blocked",
      reason: "COMPETING_SCHEDULER_ACTIVE",
    });

    return {
      outcome: "BLOCK_RESPONSE",
      controllerTriggered: "Gate0_Arbitration",
      reason: "COMPETING_SCHEDULER_ACTIVE",
      gateTrace,
    };
  }

  // 1. Tenant validity gate (must pass before anything else)
  const tenantValid = !["suspended", "trial_expired"].includes(request.tenantStatus);

  gateTrace.push({
    gate: "tenant",
    decision: tenantValid ? "passed" : "blocked",
    reason: tenantValid ? undefined : `TENANT_STATUS_${request.tenantStatus.toUpperCase()}`,
  });

  if (!tenantValid) {
    return {
      outcome: "BLOCK_RESPONSE",
      controllerTriggered: "Gate1_TenantValidity",
      reason: `TENANT_STATUS_${request.tenantStatus.toUpperCase()}`,
      gateTrace,
    };
  }
  ...
  gateTrace.push({
    gate: "safety",
    decision: safetyPassed ? "passed" : "blocked",
    reason: safetyPassed ? undefined : `SAFETY_VIOLATION: ${request.safetyFlags.join(", ")}`,
  });

  if (!safetyPassed) {
    return {
      outcome: "BLOCK_RESPONSE",
      controllerTriggered: "Gate2_Safety",
      reason: `SAFETY_VIOLATION: ${request.safetyFlags.join(", ")}`,
      gateTrace,
    };
  }

  // 3. Commitment throttling
  if (request.commitmentPending && request.activeCommitmentCount >= 1) {
    gateTrace.push({
      gate: "commitment_throttling",
      decision: "triggered",
      reason: "MAX_ACTIVE_COMMITMENTS_EXCEEDED",
    });

    return {
      outcome: "ALLOW",
      controllerTriggered: "Gate3_Throttling",
      reason: "COMMITMENT_THROTTLED",
      gateTrace,
    };
  }

  // 4. Commitment override
  if (request.commitmentPending) {
    gateTrace.push({
      gate: "commitment",
      decision: "triggered",
      reason: "PENDING_COMMITMENT_PRIORITY",
    });

    return {
      outcome: "ALLOW_WITH_CONTEXT_OVERRIDE",
      controllerTriggered: "Gate4_Commitment",
      reason: "PENDING_COMMITMENT_PRIORITY",
      gateTrace,
    };
  }
  ...
  if (["abm_schedule", "daily_brief"].includes(request.triggeredBy)) {
    gateTrace.push({
      gate: "business_routing",
      decision: "passed",
      reason: `ROUTED_BY_${request.triggeredBy.toUpperCase()}`,
    });

    return {
      outcome: "ALLOW",
      controllerTriggered: "Gate5_BusinessLogic",
      reason: `ROUTED_BY_${request.triggeredBy.toUpperCase()}`,
      gateTrace,
    };
  }
  ...
  return {
    outcome: "ALLOW",
    controllerTriggered: "Gate6_Default",
    reason: "ALL_GATES_PASSED",
    gateTrace,
  };
}
...
async function enrichVerdict(
  ...
  const needsEnrichment = ["ALLOW_WITH_CONTEXT_OVERRIDE", "QUEUE_FOR_RECOVERY"].includes(
    gateResult.outcome
  );
  ...
  if (!needsEnrichment) return baseVerdict;
...
Return enriched ArbiterVerdict.`;
    ...
    if (!validated.success) {
      return {
        ...baseVerdict,
        outcome:
          gateResult.outcome === "ALLOW_WITH_CONTEXT_OVERRIDE"
            ? "ALLOW"
            : gateResult.outcome,
        controllerTriggered: "enrichment_schema_fallback",
    ...
    return {
      ...
      outcome:
        gateResult.outcome === "ALLOW_WITH_CONTEXT_OVERRIDE"
          ? "ALLOW"
          : gateResult.outcome,
      controllerTriggered: "enrichment_fallback",
...
export async function runArbiter(
  ...
  if (
    ...
    try {
      const [lease] =
        await sql`SELECT public.lease_credit(${request.tenantId}, ${request.creditRequired}) as result`;
  ...
  try {
    await sql`
      ...
      VALUES (
        ...
        ${request.fingerprint},
        ${verdict.outcome},
        ${verdict.controllerTriggered},
        ${sql.json(verdict.gateTrace)},
        ${durationMs},
        ${request.triggeredBy}
      )
    `;

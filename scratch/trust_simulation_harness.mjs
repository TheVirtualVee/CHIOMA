...
async function runTrustSimulation() {
  ...
  try {
    ...
    const ingressResult = await runAtomicStaffLoop(initialInput, sql, { apiKey, provider }, telemetry);
    ...
    if (commitments.length === 0) {
      ...
      // We'll inject one manually to continue the test of the recovery loop if LLM was too polite
    ...
    const recoveryResult = await processOverdueCommitments(sql, { apiKey, provider });
